...
class DesignDistribution_log:
    def __init__(self, initial_means, initial_stds, lr_mean=0.001, lr_std=0.001, lr_weight = 0.001,
                 min_parameters=np.ones(8, dtype=np.float32), max_parameters=np.ones(8, dtype=np.float32),
                 max_std=np.ones(8, dtype=np.float32), min_std=np.ones(8, dtype=np.float32), weights=None):

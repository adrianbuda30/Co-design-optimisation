...
class LQREnv(utils.EzPickle, gym.Env):
    ...
    def step(self, action):
        ...
        if (self.consecutive_time > 10 or self.time >= 100) and self.checker == 0:
    ...
    def reset(self, seed=None):
        ...
        for Vinf in range(1,200):
            self.MatrixA, self.MatrixB, self.MatrixC = state_space(Vinf, self.mass, self.x_cg, self.x_ea, self.k_h, self.k_alpha)
            V_ae, U_ae = np.linalg.eig(self.MatrixA)
            ok = 0

            for index in range(12):
                if V_ae[index].real > 0 and ok == 0:
                    ok = 1
            if ok == 1 and item == 0:
                Vflut = Vinf
                item = 1

        self.MatrixA, self.MatrixB, self.MatrixC = state_space(Vflut, self.mass, self.x_cg, self.x_ea, self.k_h,
                                                               self.k_alpha)

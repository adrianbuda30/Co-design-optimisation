...
class AerofoilEnv(utils.EzPickle, gym.Env):
    ...
    def __init__(self, REWARD=np.array([0.0, 0.0]), env_id=0, mass=1.5, x_cg=0.62, x_ea=0.35, k_h=1800, k_alpha=370, k_b=390,
                 call_back=f"random_design"):
        ...
        self.MatrixA = np.zeros(64)
        self.MatrixB = np.zeros(8)
        self.MatrixC = np.zeros(8)
        self.delta_ddot = 0
    ...
    def set_design_params(self, new_design_params):
        ...
        self.x_cg = new_design_params[0]
        self.x_ea = new_design_params[1]
    ...
    def step(self, action):
        ...
        #high_action_penalty_reward = self.ACTION_PENALTY / (abs(action) + 1)
        # if self.pitch > self.MIN_PITCH and self.pitch < self.MAX_PITCH and self.delta > self.MIN_DELTA and self.delta < self.MAX_DELTA:
        #distance = abs(self.CL - self.desired_CL)
        #desired_CL_reward = self.CL_PENALTY / (distance + 1)

        #reward = desired_CL_reward + high_action_penalty_reward
        ...
        reward = - ((self.pitch - self.desired_pitch) * 180 / np.pi) ** 2
        ...
        if (self.consecutive_time > 5 or self.time >= 100) and self.checker == 0:
            self.convergence_time = self.time
            print(self.convergence_time)
            self.checker = 1

        if self.checker == 1 or early_Stop or done:
    ...
    def reset(self, seed=None):
        ...
        #    self.x_cg = 0.60 #random.uniform(self.MIN_X_CG, self.MAX_X_CG)
        #    self.x_ea = 0.35 #random.uniform(self.MIN_X_EA, self.MAX_X_EA)
